...
public class EmailUtils {
	 // 自己的邮箱
	public static String sendEmailAccount = "2412256398@qq.com";
	// 邮箱授权码
	public static String sendEmailPwd = "znsprtnkjozydjih";

   //收件人邮箱
   public  static String receiveMailAccount="2669333860@qq.com";
...
public static void    createMimeMessage(String sendMail,String receiveMail) throws UnsupportedEncodingException, MessagingException
{
  ...
  //4.发件人
  message.setFrom(new InternetAddress(sendMail, "发件人的昵称：张师傅", "UTF-8"));
  ...
  //6.设定主题
  message.setSubject("轻松购有效公司","UTF-8");
...
public static void createMimeMessage(HttpServletRequest request, String email, String ucode) throws UnsupportedEncodingException, MessagingException {
	  ...
	  // 3. 创建一封邮件
	   ...
	   //    CC:抄送人，BCC:密送
	   message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(email, "新注册的用户", "UTF-8"));

	   // 6. Subject: 邮件主题（标题有广告嫌疑，避免被邮件服务器误认为是滥发广告以至返回失败，请修改标题）
	   message.setSubject("轻松购系统用户激活码", "UTF-8");

	   // 7. Content: 邮件正文
	   String context = "<div>激活验证码:<a href='"+basePath+"jsps/User/validation.jsp'>"+basePath+"jsps/User/validation.jsp</a>，且验证码为："+ucode+"</div>";
